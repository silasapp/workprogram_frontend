...
export class ParameterConfigComponent implements OnInit {
  ...
  fetchdata() {
    this.admin.fetchparconfig().subscribe((res) => {
  ...
  addDataDuration_pw() {
    this.admin
      .addDataDuration_pw(this.dataDuration_pw_Form.getRawValue(), 'INSERT')
      .subscribe((res) => {
  ...
  addDataDuration_duw() {
    this.admin
      .addDataDuration_duw(this.dataDuration_duw_Form.getRawValue(), 'INSERT')
      .subscribe((res) => {
        if (res.statusCode == 200) {
          this.Alert('Success', res.message, 'success');
          this.fetchdata();
