...
import {
  ...
  DRILLING_OPERATIONS_CATEGORIES_OF_WELL,
} from 'src/app/models/step1-drilling.model';
import {
...
export class SWPDrillingOperationsComponent implements OnInit {
    ...
    this.CategoryForm = new FormGroup(
      {
        ...
        well_trajectory: new FormControl(this.categoryBody.well_trajectory, [
          Validators.required,
        ]),
        spud_date: new FormControl(this.categoryBody.spud_date, [
          Validators.required,
        ]),
        cored: new FormControl(this.categoryBody.cored, [Validators.required]),
        core_Depth_Interval: new FormControl(
          this.categoryBody.core_Depth_Interval,
          [Validators.required]
        ),
        core_Cost_USD: new FormControl(this.categoryBody.core_Cost_USD, [
          Validators.required,
        ]),
        any_New_Discoveries: new FormControl(
          this.categoryBody.any_New_Discoveries,
          [Validators.required]
        ),
        state_the_field_where_Discovery_was_made: new FormControl(
          this.categoryBody.state_the_field_where_Discovery_was_made,
          [Validators.required]
        ),
        hydrocarbon_Counts: new FormControl(
        ...
        processing_Fees_Paid: new FormControl(
          this.categoryBody.processing_Fees_Paid,
          [Validators.required]
        ),
        number_of_Days_to_Total_Depth: new FormControl(
          this.categoryBody.number_of_Days_to_Total_Depth,
          [Validators.required]
        ),
        terrain: new FormControl(this.categoryBody.terrain_Drill, [
          Validators.required,
        ]),
        water_depth: new FormControl(this.categoryBody.water_depth, [
          Validators.required,
        ]),
        true_vertical_depth: new FormControl(
          this.categoryBody.true_vertical_depth,
          [Validators.required]
        ),
        measured_depth: new FormControl(this.categoryBody.measured_depth, [
          Validators.required,
        ]),
        depth_refrence: new FormControl(this.categoryBody.depth_refrence, [
          Validators.required,
        ]),
        rig_type: new FormControl(this.categoryBody.rig_type, [
          Validators.required,
        ]),
        rig_Name: new FormControl(this.categoryBody.rig_Name, [
          Validators.required,
        ]),
        target_reservoir: new FormControl(this.categoryBody.target_reservoir, [
          Validators.required,
        ]),
        location_name: new FormControl(this.categoryBody.location_name, [
          Validators.required,
        ]),
        basin: new FormControl(this.categoryBody.basin, [Validators.required]),
        propose_well_names: new FormControl(
          this.categoryBody.propose_well_names,
          [Validators.required]
        ),
        actual_wells_name: new FormControl(
          this.categoryBody.actual_wells_name,
          [Validators.required]
        ),
      ...
      .subscribe((res) => {
        if (res.drillOperationCategoriesWell) {
          ...
          if (this.quaterACThreeData || this.quaterACThreeData !== undefined) {
            this.quaterACThreeData.spud_date = this.genk.formDate(
              this.quaterACThreeData.spud_date
            );
          ...
          this.quaterACFourData = res.drillOperationCategoriesWell.filter(
            (res) => {
          ...
          if (this.quaterACFourData || this.quaterACFourData !== undefined) {
            this.quaterACFourData.spud_date = this.genk.formDate(
              this.quaterACFourData.spud_date
            );
  ...
  saveHydroDoc(DeFile: any) {
    ...
    if (
      this.hydrocarbonFile.size < 1 ||
      this.hydrocarbonFile.size > 1024 * 1024 * 50
    ) {
      this.CategoryForm.controls['hydrocarbonFile'].setErrors({
        incorrect: true,
      });
    ...
    this.hydroNewName = this.gen.getExpDoc(
      this.hydrocarbonFile.name,
      this.hydrocarbonFile.type
    );
  ...
  saveDiscoveryDoc(DeFile: any) {
    ...
    if (
      this.discoveryFile.size < 1 ||
      this.discoveryFile.size > 1024 * 1024 * 50
    ) {
      this.CategoryForm.controls['discoveryFile'].setErrors({
        incorrect: true,
      });
    ...
    this.discoveryNewName = this.gen.getExpDoc(
      this.discoveryFile.name,
      this.discoveryFile.type
    );
