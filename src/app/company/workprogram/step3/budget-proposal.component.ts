...
import { budgetProposal, capexOpex } from 'src/app/models/step3-budget-proposal.model';
...
export class SWPBudgetProposalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.budgetProposalForm = new FormGroup({
      ...
      budget_for_other_Activities_Naira : new FormControl(this.budgetProposalBody.budget_for_other_Activities_Naira, Validators.required),
      budget_for_other_Activities_Dollars: new FormControl(this.budgetProposalBody.budget_for_other_Activities_Dollars, Validators.required),
      total_Company_Expenditure_Dollars: new FormControl(this.budgetProposalBody.total_Company_Expenditure_Dollars, Validators.required)
    });
    this.capexOpexForm = new FormGroup({
      item_Description: new FormControl(this.capexOpexBody.item_Description, Validators.required),
      ...
      dollar_equivalent: new FormControl(this.capexOpexBody.dollar_equivalent, Validators.required),
  ...
  debugger;
    this.workprogram
      .post_Opex(info, this.genk.wpYear, this.genk.OmlName, this.genk.fieldName, event.target.value, "DELETE")
      .subscribe(res => {
        debugger;
        if(res.statusCode == 300){
          this.modalService.logNotice("Error", res.message, 'error');
