...
export class DevelopmentWellsComponent implements OnInit {
  ...
  selectedColumns: any[] = [];
    ...
    reporttext: string = 'A total of (10) Development wells were drilled during the year 2021 - Table 10 below shows the details:';
    isChart = false;
    totalone = 0;
    totaltwo = 0;
  ...
  async plotDoublePieChart() {
    if (this.selectedColumns.length > 2) {
      ...
      if (this.selectedColumns.length === 2) {
        ...
        this.report.developmentWellsChart = await this.report.plotDoublePieChart(bechart, sele1, sele2, chartdata)
      }
      this.isChart = true;
    }
  }
    ...
    if (this.selectedColumns.length > 2) {
      ...
      if (this.selectedColumns.length === 2) {
        ...
        if (totalString.length > 70) {
          this.report.developmentWellsChart = await this.report.plotDoubleBarChartHorizontal(bechart, this.selectedColumns[0].columnDef, this.selectedColumns[1].columnDef, chartdata);
        }
        else {
          this.report.developmentWellsChart = await this.report.plotDoubleBarChart(bechart, this.selectedColumns[0].columnDef, this.selectedColumns[1].columnDef, chartdata);
        }
      }
      this.isChart = true;
    }
  ...
  transferData() {
    this.report.developmentWellsTable = {data: this.data, header: this.columns};
