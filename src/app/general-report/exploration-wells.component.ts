...
export class ExplorationWellsComponent implements OnInit {
  ...
  getExplorationWells() {
    this.workprogram
      .getExplorationWells(this.genk.reportYear)
      .subscribe((res) => {
        ...
        this.totalone = Math.round(
          this.report.sumColumn(this.data, 'number_of_Days_to_Total_Depth')
        );
        this.totaltwo = Math.round(
          this.report.sumColumn(this.data, 'well_cost')
        );
  ...
  transferData() {
    this.report.explorationWellsTable = {
      data: this.data,
      header: this.columns,
    };
