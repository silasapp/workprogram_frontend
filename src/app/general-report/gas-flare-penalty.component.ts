...
export class GasFlarePenaltyComponent implements OnInit {
  ...
  selectedColumns: any[] = [];
    ...
    title = ' GAS FLARE PENALTIES AND PAYMENTS';
    reporttext: string;
    pagenum = 0;
    selectedPage = 0;
    arrayRows = [];
    ...
    public get tableTitle(): string {
      return `Table 4 Gas Flare Volumes and Penalty Payments ${this.genk.reportYear}`;
    }
    ...
    columns = [
        ...
        {
          "columnDef": `penaltyfeepaid`,
          "header": `PENALTY FEE PAID`
        }
      ];
  ...
  getGasProductionReport() {
    this.workprogram.GetGasProductionReport(this.genk.reportYear)
      .subscribe(res => {
        this.data = res.gas_Flare_Penalty as any[];
        this.totalone = Math.round(this.report.sumColumn(this.data, `flared`));
