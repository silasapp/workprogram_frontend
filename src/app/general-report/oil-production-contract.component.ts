...
export class OilProductionContractComponent implements OnInit {
  ...
  public get tableTitle(): string {
    return (
      'Table 1b Oil Production on Contract basis for the year ' +
  ...
  filterData(data: any[]) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      myObj.percentage_Production = (
        (data[i]['_' + this.genk.reportYear] / total) *
        100
      ).toFixed(2);
      myList.push(myObj);
      myObj = {};
  ...
  transferData() {
    this.report.oilProductionContractTable = {
      data: this.data,
      header: this.columns,
    };
