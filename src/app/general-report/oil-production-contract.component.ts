...
@Component({
  selector: 'app-oil-production-contract',
...
export class OilProductionContractComponent implements OnInit {
  ...
  async plotDoublePieChart() {
    if (this.selectedColumns.length > 2) {
      ...
      if (this.selectedColumns.length === 2) {
        ...
        this.report.oilProductionContractChart = await this.report.plotDoublePieChart(bechart, sele1, sele2, chartdata)
      }
      this.isChart = true;
    }
  }
    ...
    if (this.selectedColumns.length > 2) {
      ...
      if (this.selectedColumns.length === 2) {
        ...
        if (totalString.length > 70) {
          this.report.oilProductionContractChart = await this.report.plotDoubleBarChartHorizontal(bechart, this.selectedColumns[0].columnDef, this.selectedColumns[1].columnDef, chartdata);
        }
        else {
          this.report.oilProductionContractChart = await this.report.plotDoubleBarChart(bechart, this.selectedColumns[0].columnDef, this.selectedColumns[1].columnDef, chartdata);
        }
      }
      this.isChart = true;
    }
  ...
  transferData() {
    this.report.oilProductionContractTable = {data: this.data, header: this.columns};
    this.report.oilProductionContractIsChart = this.isChart;
    this.report.oilProductionContractSelectedColumns = this.selectedColumns;
  }
