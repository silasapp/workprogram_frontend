...
@Component({
  selector: 'app-oil-production-terrain',
...
export class OilProductionTerrainComponent implements OnInit {
  ...
  async plotDoublePieChart() {
    if (this.selectedColumns.length > 2) {
      ...
      if (this.selectedColumns.length === 2) {
        ...
        this.report.oilProductionTerrainChart = await this.report.plotDoublePieChart(bechart, sele1, sele2, chartdata)
      }
      this.isChart = true;
    }
  }
    ...
    if (this.selectedColumns.length > 2) {
      ...
      if (this.selectedColumns.length === 2) {
        ...
        if (totalString.length > 70) {
          this.report.oilProductionTerrainChart = await this.report.plotDoubleBarChartHorizontal(bechart, this.selectedColumns[0].columnDef, this.selectedColumns[1].columnDef, chartdata);
        }
        else {
          this.report.oilProductionTerrainChart = await this.report.plotDoubleBarChart(bechart, this.selectedColumns[0].columnDef, this.selectedColumns[1].columnDef, chartdata);
        }
      }
      this.isChart = true;
    }
  ...
  transferData() {
    this.report.oilProductionTerrainTable = {data: this.data, header: this.columns};
    this.report.oilProductionTerrainIsChart = this.isChart;
    this.report.oilProductionTerrainSelectedColumns = this.selectedColumns;
  }
