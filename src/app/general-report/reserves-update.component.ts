...
export class ReserveUpdateComponent implements OnInit {
  ...
  public get reporttext(): string {
    return (
      'The table below shows reserves update as at 1st January ' +
  ...
  getReservesUpdatestWells() {
    this.workprogram
      .getReservesUpdatestWells(this.genk.reportYear)
      .subscribe((res) => {
        ...
        this.totalone = Math.round(
          this.report.sumColumn(this.data, 'total_Company_Reserves_Oil')
        );
        this.totaltwo = Math.round(
          this.report.sumColumn(this.data, 'total_Company_Reserves_Condensate')
        );
        this.cd.markForCheck();
        this.assignDataRows();
        this.assignPageNum();
