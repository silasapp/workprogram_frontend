...
export class SeismicActivitiesApprovedComponent implements OnInit {
  ...
  getSeismic() {
    if (this.genk.reportYear !== undefined || this.genk.reportYear != null) {
      ...
      this.workprogram.getSeismicActivities(this.genk.reportYear).subscribe(
        (res) => {
          ...
          this.totaltwo = Math.round(
            this.report.sumColumn(this.data, 'quantum')
          );
          this.assignDataRows();
          this.assignPageNum();
  ...
  getSeismicReportText() {
    this.workprogram
      .getSeismicActivitiesReportText(this.genk.reportYear)
      .subscribe((res) => {
  ...
  transferData() {
    this.report.seismicActivitiesTable = {
      data: this.data,
      header: this.columns,
    };
