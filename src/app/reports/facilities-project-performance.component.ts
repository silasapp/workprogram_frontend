...
@Component({
  selector: 'app-facilities-project-performance',
  templateUrl: 'ndr-report.component.html',
   styleUrls: ['./ndr-report.component.scss', '../general-report/general-report.component.scss'],

  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FacilitiesProjectPerformanceComponent implements OnInit {
@ViewChild('mychart', { static: false }) myChart: ElementRef<HTMLDivElement>; 
  ...
  title = 'FACILITIES PROJECT PERFORMANCE';
  pagenum = 0;
  selectedPage = 1;
  arrayRows = [];
  data: any[];
  year = [];
   ...
   columns = [
    ...
    fetchdata(e){
      ...
      this.report.fetch("facilities_project_performance", value).subscribe(
        (res) => {
    ...
    yearList() {
      this.report.getYearList("facilities_project_performance_yearlist")
          .subscribe((res: any[]) => {
              this.year = res;
              this.cd.markForCheck();
          });
