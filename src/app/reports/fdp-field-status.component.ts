...
export class FDPFieldStatusComponent implements OnInit {
  ...
  title = 'NAMES OF FIELDS AND INDICATE ITS FIELD DEVELOPMENT PLAN STATUS';
  pagenum = 0;
  selectedPage = 1;
  arrayRows = [];
  data: any[];
  ...
  columns = [
    ...
    {
      columnDef: 'development_plan_status',
  ...
  fetchdata(e) {
    ...
    this.report.fetch('fdp_fieldstatus', value).subscribe((res) => {
  ...
  yearList() {
    this.report
      .getYearList('fdp_fieldstatus_yearlist')
      .subscribe((res: any[]) => {
        this.year = res;
        this.cd.markForCheck();
      });
