...
@Component({
  selector: 'app-gas-production-activities-domestic-supply',
...
export class GasProductionActivitiesDomesticSupplyComponent implements OnInit {
  ...
  title = 'GAS PRODUCTION ACTIVITIES DOMESTIC SUPPLY';
  pagenum = 0;
  selectedPage = 1;
  arrayRows = [];
  data: any[];
  ...
  fetchdata(e) {
    ...
    this.report
      .fetch('gas_production_activities_domestic_supply', value)
      .subscribe((res) => {
  ...
  yearList() {
    this.report
      .getYearList('gas_production_activities_domestic_supply_yearlist')
      .subscribe((res: any[]) => {
        this.year = res;
        this.cd.markForCheck();
      });
