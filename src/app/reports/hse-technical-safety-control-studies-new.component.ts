...
@Component({
  selector: 'app-hse-technical-safety-control-studies-new',
...
export class HSETechnicalSafetyControlStudiesNewComponent implements OnInit {
  ...
  title = 'HSE TECHNICAL SAFETY CONTROL STUDIES NEW';
  pagenum = 0;
  selectedPage = 1;
  arrayRows = [];
  data: any[];
  ...
  fetchdata(e) {
    ...
    this.report
      .fetch('hse_technical_safety_control_studies_new', value)
      .subscribe((res) => {
  ...
  yearList() {
    this.report
      .getYearList('hse_technical_safety_control_studies_new_yearlist')
      .subscribe((res: any[]) => {
        this.year = res;
        this.cd.markForCheck();
      });
