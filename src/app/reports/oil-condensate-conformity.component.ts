...
@Component({
  selector: 'app-oil-condensate-conformity',
...
export class OilCondensateConformityComponent implements OnInit {
  ...
  fetchdata(e) {
    ...
    let result = this.report
      .fetch('oil_condensate_conformity', value)
      .subscribe((res) => {
  ...
  yearList() {
    this.report
      .getYearList('oil_condensate_conformity_yearlist')
      .subscribe((res: any[]) => {
        this.year = res;
        this.cd.markForCheck();
      });
