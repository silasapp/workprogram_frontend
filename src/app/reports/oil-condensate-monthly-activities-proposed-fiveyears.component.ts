...
@Component({
  selector: 'app-oil-condensate-monthly-activities-proposed-fiveyears',
...
export class OilCondensateMonthlyActivitiesProposedFiveyearsComponent
  implements OnInit
{
  ...
  title = 'FIVE YEAR OIL PRODUCTION FORECAST';
  pagenum = 0;
  selectedPage = 1;
  arrayRows = [];
  data: any[];
  ...
  columns = [
    ...
    {
      columnDef: 'forecast',
      header: 'Yearly Production Forcast',
    },
  ];
  ...
  fetchdata(e) {
    ...
    let result = this.report
      .fetch('oil_condensate_monthly_activities_proposed_fiveyears', value)
      .subscribe((res) => {
  ...
  yearList() {
    this.report
      .getYearList(
        'oil_condensate_monthly_activities_proposed_fiveyears_yearlist'
      )
      .subscribe((res: any[]) => {
        this.year = res;
        this.cd.markForCheck();
