...
@ViewChild('mychart', { static: false }) myChart: ElementRef<HTMLDivElement>; 
  ...
  year = [];
    ...
    fetchdata(e){
      ...
      this.report.fetch("reserves_life_index", value).subscribe(
        (res) => {
    ...
    yearList() {
      this.report.getYearList("reserves_life_index_yearlist")
          .subscribe((res: any[]) => {
              this.year = res;
              this.cd.markForCheck();
          });
