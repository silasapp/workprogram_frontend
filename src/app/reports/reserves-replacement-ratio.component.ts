...
@Component({
  selector: 'app-reserves-replacement-ratio',
  templateUrl: 'ndr-report.component.html',
   styleUrls: ['./ndr-report.component.scss', '../general-report/general-report.component.scss'],

  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ReservesReplacementRatioComponent implements OnInit {
@ViewChild('mychart', { static: false }) myChart: ElementRef<HTMLDivElement>; 
  ...
  title = 'RESERVES REPLACEMENT RATIO';
  pagenum = 0;
  selectedPage = 1;
  arrayRows = [];
  data: any[];
  year = [];
    ...
    fetchdata(e){
      ...
      this.report.fetch("reserves_replacement_ratio", value).subscribe(
        (res) => {
    ...
    yearList() {
      this.report.getYearList("reserves_replacement_ratio_yearlist")
          .subscribe((res: any[]) => {
              this.year = res;
              this.cd.markForCheck();
          });
