...
export class AuthenticationService {
   ...
   login(email: string, password: string) {
    ...
    dregister(solids: HttpParams) {
        return this.http.post<any>(`${environment.apiUrl}/account/placeDey`, solids)
            .pipe(retry(this.num), map(user => {
                // store user details and jwt token in local storage to keep user logged in between page refreshes
                localStorage.setItem('currentUser', JSON.stringify(user));
                localStorage.setItem('iden', user.id)
    ...
    verifyCode(code: string) {
        return this.http.get<any>(`${environment.apiUrl}/account/verifyCompanyCode`, { params: {companyCode: code}})
            .pipe(retry(this.num));
    }

    changeCode(oldcode: string, email: string, newcode: string) {
        return this.http.post<any>(`${environment.apiUrl}/account/checkNewPinCode`, '', { params: {oldCompanyCode: oldcode, email: email, newCompanyCode: newcode}})
            .pipe(retry(this.num));
    }

    createUser(companyName: string, companyCode: string, name: string, designation: string, phone: string, email: string, password: string) {
        return this.http.post<any>(`${environment.apiUrl}/account/createCompanyResource`, {companyName: companyName, companyCode: companyCode, name: name, designation: designation, phone: phone, email: email, password: password})
            .pipe(retry(this.num));
    }
    ...
    deleteCompanyResource(id: string, companyCode: string) {
        return this.http.post<any>(`${environment.apiUrl}/account/deleteCompanyResource`, '', {params: {id: id, companyCode: companyCode}})
            .pipe(retry(this.num));
    }

    returnPasswordInfo(email: string) {
        return this.http.post<any>(`${environment.apiUrl}/account/returnPasswordInfo`, '', {params: {email: email}})
            .pipe(retry(this.num));
    }

    changePassword(userId: string, token: string, password: string) {
        return this.http.get<any>(`${environment.apiUrl}/account/changePassword`, { params: {userId: userId, token: token, password: password}})
            .pipe(retry(this.num));
    }
