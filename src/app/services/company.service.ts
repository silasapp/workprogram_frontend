...
export class CompanyService {
  ...
  constructor(private http: HttpClient, private auth: AuthenticationService) {
    this.currentCompanySubject = new BehaviorSubject<CompanyDetails>(
  ...
  opl() {
    return this.http
      .get<any>(`${environment.apiUrl}/workprogramme/opl_recalibrated_scaled`, {
        params: { year: '2020' },
      })
      .pipe(
        retry(this.num),
  ...
  getPresentation() {
    return this.http
      .get<any>(`${environment.apiUrl}/presentation/GET_PRESENTATION_DATETIME`)
      .pipe(
        retry(this.num),
        map((response) => {
          debugger;
  ...
  schedulePresentation(time: string, date: string) {
    return this.http
      .post<any>(
        `${environment.apiUrl}/presentation/schedulepresentation`,
        '',
  ...
  getdashboardreport(year: string) {
    return this.http
      .get<any>(`${environment.apiUrl}/dashboard/company_dashboard_report`, {
        params: { year: year },
      })
      .pipe(
        retry(this.num),
  ...
  getCompanyProd(year: string) {
    return this.http
      .get<any>(`${environment.apiUrl}/dashboard/COMPANY_MONTHLY_PRODUCTION`, {
        params: { year: year },
      })
      .pipe(
        retry(this.num),
  ...
  getCompanyConcessionProd(year: string) {
    return this.http
      .get<any>(
        `${environment.apiUrl}/dashboard/COMPANY_CONCESSION_PRODUCTION`,
        { params: { year: year } }
      )
      .pipe(
        retry(this.num),
  ...
  getCompanyConcessionReserveOil(year: string) {
    return this.http
      .get<any>(`${environment.apiUrl}/dashboard/COMPANY_CONCESSION_RESERVE`, {
        params: { year: year },
      })
      .pipe(
        retry(this.num),
  ...
  getCompanyConcessionReserveGas(year: string) {
    return this.http
      .get<any>(
        `${environment.apiUrl}/dashboard/COMPANY_CONCESSION_RESERVE_GAS`,
        { params: { year: year } }
      )
      .pipe(
        retry(this.num),
  ...
  getdashboardgasbudgetandreserve(year: string) {
    return this.http
      .get<any>(`${environment.apiUrl}/dashboard/TOTAL_RESERVE_BUDGET`, {
        params: { year: year },
      })
      .pipe(
        retry(this.num),
  ...
  changePassword(e: any) {
    return this.http
      .post<any>(`${environment.apiUrl}/account/ResetPassword`, '', {
        params: {
