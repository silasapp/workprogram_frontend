...
export class ReportService {
  ...
  getProcessFlow() {
    return this.http.get<any>(
      `${environment.apiUrl}/application/getProcessFlow`
    );
  }
  ...
  getAdminChartData(url, year) {
    return this.http
      .get<any>(`${environment.apiUrl}/dashboard/${url}`, {
        params: { year: year },
      })
      .pipe(
        retry(this.num),
  ...
  getExecutiveReport(year: string) {
    return this.http
      .get<any>(`${environment.apiUrl}/report/Get_General_SummaryReport`, {
        params: { year: year },
      })
      .pipe(retry(this.num));
  }
  ...
  async plotDoubleBarChartHorizontal(
    chartdiv: HTMLDivElement,
    categoryfield: string,
    valuefield: string,
    data: any[]
...
convertDataToArray(data: any[]) {
  ...
  sumColumn(items: any[], columnName: string) {
    ...
    for (var item in items) {
      if (
        !isNaN(items[item][columnName] && parseFloat(items[item][columnName]))
      ) {
...
convertColumn(items: any[], columnName: string) {
  ...
  arrangeDate(mydata: any[], columnName: string) {
    ...
    while (i < mydata.length) {
      ...
      mydata[i][columnName] = datePipe.transform(
        mydata[i][columnName],
        'dd MMM, y'
      );
      i++;
